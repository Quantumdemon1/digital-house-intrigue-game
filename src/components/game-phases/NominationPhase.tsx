...
const NominationPhase: React.FC = () => {
  const { gameState, dispatch, getHouseguestById, showToast, logger } = useGame();
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (nominees.length < 2) {
          const randomNominees = eligibleHouseguests
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="bg-slate-100 p-4 rounded-lg flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-semibold">{hoh?.name || 'No HoH'}</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
              {nominees.map(nominee => (
                <div key={nominee?.id} className="bg-red-50 border border-red-100 rounded-lg p-4 text-center shadow-sm">
