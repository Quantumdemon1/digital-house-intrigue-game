...
const VotingStatus: React.FC<VotingStatusProps> = ({
  ...
  return <div className="mt-4 border border-gray-200 rounded-md p-4">
      <h4 className="text-sm font-medium mb-2">Voting Status</h4>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {voters.map(voter => {
        const hasVoted = Object.keys(votes).includes(voter.id);
        return <div key={voter.id} className={`flex items-center gap-1 p-2 rounded-md ${hasVoted ? 'bg-green-50' : 'bg-gray-50'}`}>
