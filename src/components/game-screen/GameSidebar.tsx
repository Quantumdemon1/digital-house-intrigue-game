...
const GameSidebar: React.FC = () => {
  ...
  const inactiveHouseguests = gameState.houseguests.filter(h => h.status !== 'Active')
    .sort((a, b) => (a.status === 'Jury' ? -1 : 1)); // Show jury above evicted
          ...
          {inactiveHouseguests.length > 0 && (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-3">
                {inactiveHouseguests.map(houseguest => (
                  <HouseguestCard 
                    key={houseguest.id} 
                    houseguest={houseguest}
                    showRelationship={true}
